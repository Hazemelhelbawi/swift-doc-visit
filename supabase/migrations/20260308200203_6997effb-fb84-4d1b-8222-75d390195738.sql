...
-- Drop all RESTRICTIVE policies on clinics and recreate as PERMISSIVE
...
-- Drop all RESTRICTIVE policies on schedules and recreate as PERMISSIVE
...
CREATE POLICY "Doctors can manage their appointments" ON public.appointments FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) AND doctor_id = get_doctor_id_for_user(auth.uid())) WITH CHECK (has_role(auth.uid(), 'admin'::app_role) AND doctor_id = get_doctor_id_for_user(auth.uid()));
...
-- Drop all RESTRICTIVE policies on consultation_requests and recreate as PERMISSIVE
...
CREATE POLICY "Doctors can manage their own consultations" ON public.consultation_requests FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) AND doctor_id = get_doctor_id_for_user(auth.uid())) WITH CHECK (has_role(auth.uid(), 'admin'::app_role) AND doctor_id = get_doctor_id_for_user(auth.uid()));

-- Drop all RESTRICTIVE policies on site_settings and recreate as PERMISSIVE
...
CREATE POLICY "Doctors can manage their own settings" ON public.site_settings FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) AND doctor_id = get_doctor_id_for_user(auth.uid())) WITH CHECK (has_role(auth.uid(), 'admin'::app_role) AND doctor_id = get_doctor_id_for_user(auth.uid()));
...
-- Drop all RESTRICTIVE policies on doctors and recreate as PERMISSIVE
