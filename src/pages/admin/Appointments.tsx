  ...
  const { data: appointments, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("appointments")
        .select(
          "*, schedules(date, start_time, end_time, clinics(name, name_ar))",
        )
                  ...
                  {appointments?.map((appointment) => (
                    <TableRow key={appointment.id}>
                        ...
                        {appointment.schedules ? (
                          <div className="text-sm">
                            ...
                            <div className="text-muted-foreground">
                              {formatTime(appointment.schedules.start_time)} -{" "}
                              {formatTime(appointment.schedules.end_time)}
                            </div>
                          </div>
                        ) : (
                          "-"
                      ...
                      <TableCell>
                        ...
                        >
                          <SelectTrigger
                            className={`w-32 ${getStatusColor(appointment.status)}`}
                          >
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
